...
warn_about_relative_paths(or_options_t *options)
{
  ...
  for (hs_line = options->RendConfigLines; hs_line;
...
load_torrc_from_disk(config_line_t *cmd_arg, int defaults_file)
{
  ...
  /* Open config file */
  st = fname ? file_status(fname) : FN_EMPTY;
  if (fname == NULL ||
...
options_init_from_torrc(int argc, char **argv)
{
  ...
  config_line_t *cmdline_only_options = NULL;
	const config_line_t *f_line ;
	int read_torrc_from_stdin;
  ...
  if (command == CMD_HASH_PASSWORD) {
    ...
    f_line = config_line_find(cmdline_only_options,
                                                   "-f");

    read_torrc_from_stdin =
...
port_cfg_new(size_t namelen)
{
  ...
  cfg = (port_cfg_t *)tor_malloc_zero(sizeof(port_cfg_t) + namelen + 1);
    ...
    if (unix_socket_path) {
      ...
      if (!strcmp(addrport, "0"))
        ...
        port = 1;
			...
			int af;
      port = CFG_AUTO_PORT;
      af = tor_addr_parse(&addr, defaultaddr);
      tor_assert(af >= 0);
            ...
            int *world_writable_control_socket)
{
  ...
  *n_ports_out = 0;
  ...
  gw_flag = options->SocksSocketsGroupWritable ?
