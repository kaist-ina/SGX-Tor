...
//For eval
//#define EVAL_BOOTSTRAPPING_ORI

#ifdef EVAL_BOOTSTRAPPING_ORI
...
queue_control_event_string,(uint16_t event, char *msg))
{
  ...
  if (PREDICT_UNLIKELY( ! EVENT_IS_INTERESTING(event) )) {
	...
	block_event_queue = get_block_event_queue();
	if (*block_event_queue) {
		tor_free(msg);
		return;
	}

  ev = tor_malloc(sizeof(*ev));
  ev->event = event;
  ev->msg = msg;
...
{
	smartlist_t * all_conns, *controllers, *queued_events;
	int *block_event_queue;
                              ...
                              const char *body)
{
  ...
  cmdline = tor_memdup_nulterm(body, cp-body);
                        ...
                        char **err_msg_out)
{
  ...
  key_type = smartlist_get(key_args, 0);
  key_blob = smartlist_get(key_args, 1);
                          ...
                          const char *body)
{
  ...
  service_id = smartlist_get(args, 0);
   ...
   * should be returned.
   */
  services[0]= conn->ephemeral_onion_services;
  services[1]= detached_onion_services;
  
  for (i = 0; i < ARRAY_LENGTH(services); i++) {
  ...
  if (onion_services == NULL) {
    ...
    cp = smartlist_get(onion_services, idx);
...
get_bw_samples(void)
{
  ...
  for (i = 0; i < n_measurements; ++i) {
		struct cached_bw_event_s *bwe;
    tor_assert(0 <= idx && idx < N_BW_EVENTS_TO_CACHE);
    bwe = &cached_bw_events[idx];
...
control_event_bootstrap(bootstrap_status_t status, int progress)
{
  ...
  if (status > bootstrap_percent ||
	...
	if(progress == 0) {
		...
		end_time = clock();
		interval = (end_time-start_time)/(double)CLOCKS_PER_SEC;
		printf("Time = %lf\n", interval);
		system("pause");
	}
