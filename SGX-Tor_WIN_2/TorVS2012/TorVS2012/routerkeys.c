           ...
           int twice, const or_options_t *options)
{
  ...
  while (1) {
    ...
    length2 = tor_getpass(prompt2, buf2, buflen);
                          ...
                          const char *fname)
{
  ...
  while (1) {
    ...
    r = crypto_unpwbox(&secret, &secret_len,
                      ...
                      struct tor_cert_st **cert_out)
{
  ...
  ed25519_keypair_t *keypair;
	int have_secret;
	int load_secret;
	int have_encrypted_secret_file;
	int found_public;
	uint8_t certbuf[256];
	uint32_t cert_flags;
           ...
           struct tor_cert_st **cert_out)
{
  ...
  extra_strong = !! (flags & INIT_ED_KEY_EXTRA_STRONG);
  keypair = tor_malloc_zero(sizeof(ed25519_keypair_t));
...
load_ed_keys(const or_options_t *options, time_t now)
{
  ...
  tor_cert_t *auth_cert = NULL;
	int offline_master;
	int need_new_signing_key;
	int want_new_signing_key;
	int can_make_master_id_key ;
#define FAIL(msg) do {                          \
    log_warn(LD_OR, (msg));                     \
    goto err;                                   \
  } while (0)
...
#define EXPIRES_SOON(cert, interval)            \
  ...
  can_make_master_id_key = !offline_master &&
    NULL == use_signing;
...
generate_ed_link_cert(const or_options_t *options, time_t now)
{
  ...
  digests = (digests_t *)tor_x509_cert_get_cert_digests(link);
                             ...
                             int *len_out)
{
  ...
  *len_out = 0;
  ...
  r = crypto_pk_private_sign(onion_key,
