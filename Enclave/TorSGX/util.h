...
void *real_tor_malloc_(size_t size DMALLOC_PARAMS) ATTR_MALLOC;
void *tor_malloc_zero_(size_t size DMALLOC_PARAMS) ATTR_MALLOC;
void *real_tor_malloc_zero_(size_t size DMALLOC_PARAMS) ATTR_MALLOC;
void *tor_calloc_(size_t nmemb, size_t size DMALLOC_PARAMS) ATTR_MALLOC;
void *real_tor_calloc_(size_t nmemb, size_t size DMALLOC_PARAMS) ATTR_MALLOC;
void *tor_realloc_(void *ptr, size_t size DMALLOC_PARAMS);
void *real_tor_realloc_(void *ptr, size_t size DMALLOC_PARAMS);
void *tor_reallocarray_(void *ptr, size_t size1, size_t size2 DMALLOC_PARAMS);
void *real_tor_reallocarray_(void *ptr, size_t size1, size_t size2 DMALLOC_PARAMS);
...
void *tor_memdup_(const void *mem, size_t len DMALLOC_PARAMS)
  ATTR_MALLOC ATTR_NONNULL((1));
void *real_tor_memdup_(const void *mem, size_t len DMALLOC_PARAMS)
  ATTR_MALLOC ATTR_NONNULL((1));
...
#define real_tor_free(p) STMT_BEGIN				                   \
if (PREDICT_LIKELY((p) != NULL)) {														\
	sgx_free(p);																		                 \
	(p) = NULL;																		               \
}																									             \
...
#define real_tor_malloc(size)       real_tor_malloc_(size DMALLOC_ARGS)
#define tor_malloc_zero(size)  tor_malloc_zero_(size DMALLOC_ARGS)
#define real_tor_malloc_zero(size)  real_tor_malloc_zero_(size DMALLOC_ARGS)
#define tor_calloc(nmemb,size) tor_calloc_(nmemb, size DMALLOC_ARGS)
#define real_tor_calloc(nmemb,size) real_tor_calloc_(nmemb, size DMALLOC_ARGS)
#define tor_realloc(ptr, size) tor_realloc_(ptr, size DMALLOC_ARGS)
#define real_tor_realloc(ptr, size) real_tor_realloc_(ptr, size DMALLOC_ARGS)
#define tor_reallocarray(ptr, sz1, sz2) \
  tor_reallocarray_((ptr), (sz1), (sz2) DMALLOC_ARGS)
#define real_tor_reallocarray(ptr, sz1, sz2) \
	real_tor_reallocarray_((ptr), (sz1), (sz2)DMALLOC_ARGS)
...
#define real_tor_memdup(s, n)       real_tor_memdup_(s, n DMALLOC_ARGS)
...
ssize_t real_write_all(tor_socket_t fd, const char *buf, size_t count,int isSocket);
...
file_status_t real_file_status(const char *fname);
...
typedef struct real_open_file_t real_open_file_t;
int start_writing_to_file(const char *fname, int open_flags, int mode,
                          open_file_t **data_out);
int real_start_writing_to_file(const char *fname, int open_flags, int mode,
...
int real_abort_writing_to_file(real_open_file_t *file_data);
int write_str_to_file(const char *fname, const char *str, int bin);
int real_write_str_to_file(const char *fname, const char *str, int bin);
MOCK_DECL(int,
...
int real_write_chunks_to_file(const char *fname, const struct smartlist_t *chunks,
                         int bin, int no_tempfile);
int real_append_bytes_to_file(const char *fname, const char *str, size_t len,
                         int bin);
int real_write_bytes_to_new_file(const char *fname, const char *str, size_t len,
                            int bin);
...
char *read_file_to_str(const char *filename, int flags, struct stat *stat_out)
  ATTR_MALLOC;
char *real_read_file_to_str(const char *filename, int flags, struct stat *stat_out)
  ATTR_MALLOC;
char *read_file_to_str_until_eof(int fd, size_t max_bytes_to_read,
                                 size_t *sz_out)
  ATTR_MALLOC;
char *real_read_file_to_str_until_eof(int fd, size_t max_bytes_to_read,
                                 size_t *sz_out)
  ATTR_MALLOC;
