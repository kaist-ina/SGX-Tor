...
//  if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (void*) &one,
  if (sgx_setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (void*) &one,
...
tor_listen(tor_socket_t fd)
{
  ...
  if ((r = sgx_listen(fd, listen_limit)) < 0) {
    if (listen_limit == SOMAXCONN)
      return r;
    if ((r = sgx_listen(fd, SOMAXCONN)) == 0) {
                        ...
                        const port_cfg_t *port_cfg)
{
  ...
  if (listensockaddr->sa_family == AF_INET ||
    ...
    if (sgx_bind(s,listensockaddr,socklen) < 0) {
...
connection_handle_listener_read(connection_t *conn, int new_type)
{
  ...
  if (!SOCKET_OK(news)) { /* accept() error */
	...
	// SGX: Check remote attestation accept list
	if(get_options()->AuthoritativeDir && new_type == CONN_TYPE_OR) {
		printf("Remote attestation check list checking start!\n");
		if(sgx_check_remote_accept_list(((struct sockaddr_in *)remote)->sin_addr.s_addr) != 0) {
			printf("Remote attestation checking failed!\n");
			connection_mark_for_close(conn);
			return -1;
		}
		printf("Remote attestation checking success!\n");
	}
                            ...
                            int *socket_error)
{
  ...
  if (bindaddr && sgx_bind(s, bindaddr, bindaddr_len) < 0) {
    *socket_error = tor_socket_errno(s);
  ...
  struct sockaddr_in * tmp = (struct sockaddr_in *)sa;
	printf("Connect = %u.%u.%u.%u:%d\n", sa->sa_data[2]&0xFF,sa->sa_data[3]&0xFF,sa->sa_data[4]&0xFF,sa->sa_data[5]&0xFF, ntohs(tmp->sin_port));
                   ...
                   const tor_addr_t *addr, uint16_t port, int *socket_error)
{
  ...
  tor_assert(dest_addr_len > 0);
	...
	// SGX: Check remote attestation accept list
	if (get_options()->AuthoritativeDir && conn->type == CONN_TYPE_OR) {
		printf("Remote attestation check list checking start!\n");
		if (sgx_check_remote_accept_list(((struct sockaddr_in *)dest_addr)->sin_addr.s_addr) != 0) {
			printf("Remote attestation checking failed!\n");
			return -1;
		}
		printf("Remote attestation checking success!\n");
	...
	printf("Connecting to %s:%u.",escaped_safe_str_client(address), port);
...
connection_handle_write_impl(connection_t *conn, int force)
{
  ...
  if (connection_state_is_connecting(conn)) {
    if (sgx_getsockopt(conn->s, SOL_SOCKET, SO_ERROR, (void*)&e, &len) < 0) {
...
set_constrained_socket_buffers(tor_socket_t sock, int size)
{
  ...
  if (sgx_setsockopt(sock, SOL_SOCKET, SO_SNDBUF, sz, sz_sz) < 0) {
    int e = tor_socket_errno(sock);
    log_warn(LD_NET, "setsockopt() to constrain send "
             "buffer to %d bytes failed: %s", size, tor_socket_strerror(e));
  }
  if (sgx_setsockopt(sock, SOL_SOCKET, SO_RCVBUF, sz, sz_sz) < 0) {
    int e = tor_socket_errno(sock);
