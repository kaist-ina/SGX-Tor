...
//    if (fcntl(fd, F_SETFD, FD_CLOEXEC) == -1) {
    if (sgx_fcntl(fd, F_SETFD, FD_CLOEXEC) == -1) {
...
int
real_tor_open_cloexec(const char *path, int flags, unsigned mode)
{
  int fd;
  const char *p = sandbox_intern_string(path);
#ifdef O_CLOEXEC
  ...
  log_debug(LD_FS, "Opening %s with flags %x", p, flags);
  fd = real_sgx_open(p, flags, mode);
#ifdef FD_CLOEXEC
  if (fd >= 0) {
//    if (fcntl(fd, F_SETFD, FD_CLOEXEC) == -1) {
    if (real_sgx_fcntl(fd, F_SETFD, FD_CLOEXEC) == -1) {
      log_warn(LD_FS,"Couldn't set FD_CLOEXEC: %s", strerror(errno));
      close(fd);
      return -1;
    }
...
//    if (fcntl(fileno(result), F_SETFD, FD_CLOEXEC) == -1) {
    if (sgx_fcntl(fileno(result), F_SETFD, FD_CLOEXEC) == -1) {
      log_warn(LD_FS,"Couldn't set FD_CLOEXEC: %s", strerror(errno));
      fclose(result);
      return NULL;
    }
...
tor_rename(const char *path_old, const char *path_new)
{
  ...
  return sgx_rename(sandbox_intern_string(path_old), sandbox_intern_string(path_new));
}

int
real_tor_rename(const char *path_old, const char *path_new)
{
  log_debug(LD_FS, "Renaming %s to %s", path_old, path_new);
  return real_sgx_rename(sandbox_intern_string(path_old), sandbox_intern_string(path_new));
}
...
tor_mmap_file(const char *filename)
{
...
#endif
	f = sgx_get_file(tfilename);
	if(f == NULL)
		goto win_err;

	res->size = f->content_len;
  res->data = f->content;
  if (!res->data)
    goto win_err;
  ...
  return res;
 win_err: {
    log_fn(LOG_INFO, LD_FS, "Couldn't mmap file \"%s\"", filename);
    errno = ENOENT;
  }
 err:
  tor_munmap_file(res);
...
tor_mmap_t *
real_tor_mmap_file(const char *filename)
{
  TCHAR tfilename[MAX_PATH]= {0};
  ...
  strlcpy(tfilename,filename,MAX_PATH);
#endif
  file_handle = real_sgx_CreateFile(tfilename,
  ...
  size_low = real_sgx_GetFileSize(file_handle, &size_high);

  if (size_low == INVALID_FILE_SIZE && sgx_GetLastError() != NO_ERROR) {
  ...
  res->mmap_handle = real_sgx_CreateFileMapping(file_handle,
                                       NULL,
                                       PAGE_READONLY,
                                       size_high,
                                       size_low,
  ...
  if (res->mmap_handle == NULL)
    goto win_err;
  res->data = (char*) real_sgx_MapViewOfFile(res->mmap_handle,
                                    FILE_MAP_READ,
                                    0, 0, 0);
  if (!res->data)
    goto win_err;

  sgx_CloseHandle(file_handle);
  return res;
 win_err: {
    DWORD e =  sgx_GetLastError();
 ...
 err:
  ...
  if (file_handle != INVALID_HANDLE_VALUE)
    sgx_CloseHandle(file_handle);
  real_tor_munmap_file(res);
  return NULL;
}

/* Unmap the file, and return 0 for success or -1 for failure */
int
real_tor_munmap_file(tor_mmap_t *handle)
{
  if (handle == NULL)
    return 0;
  ...
  if (handle->data) {
    ...
    BOOL ok = real_sgx_UnmapViewOfFile( (LPVOID) handle->data);
    if (!ok) {
      log_warn(LD_FS, "Failed to UnmapViewOfFile() in tor_munmap_file(): %d",
               (int)sgx_GetLastError());
    }
  }

  if (handle->mmap_handle != NULL)
    sgx_CloseHandle(handle->mmap_handle);
  tor_free(handle);
...
int
real_replace_file(const char *from, const char *to)
{
#ifndef _WIN32
  return tor_rename(from, to);
#else
  switch (real_file_status(to))
    {
    case FN_NOENT:
      break;
    case FN_FILE:
    case FN_EMPTY:
      if (real_sgx_unlink(to)) return -1;
      break;
    case FN_ERROR:
      return -1;
    case FN_DIR:
  ...
  return real_tor_rename(from,to);
#endif
}
...
tor_lockfile_lock(const char *filename, int blocking, int *locked_out)
{
  ...
  *locked_out = 0;
  ...
  fd = real_tor_open_cloexec(filename, O_RDWR|O_CREAT|O_TRUNC, 0600);
  if (fd < 0) {
...
#ifdef _WIN32
  real_sgx_lseek(fd, 0, SEEK_SET);
  if (real_sgx_locking(fd, blocking ? _LK_LOCK : _LK_NBLCK, 1) < 0) {
...
//    if (fcntl(fd, blocking ? F_SETLKW : F_SETLK, &lock) < 0) {
    if (real_sgx_fcntl2(fd, blocking ? F_SETLKW : F_SETLK, &lock) < 0) {
...
//  return (off_t) lseek(fd, 0, SEEK_CUR);
  return (off_t) real_sgx_lseek(fd, 0, SEEK_CUR);
#endif
}

off_t
real_tor_fd_getpos(int fd)
{
#ifdef _WIN32
  return (off_t) real_sgx_lseek(fd, 0, SEEK_CUR);
#else
...
int
real_tor_fd_seekend(int fd)
{
#ifdef _WIN32
  return real_sgx_lseek(fd, 0, SEEK_END) < 0 ? -1 : 0;
#else
  off_t rc = lseek(fd, 0, SEEK_END) < 0 ? -1 : 0;
#ifdef ESPIPE
  /* If we get an error and ESPIPE, then it's a pipe or a socket of a fifo:
...
int
real_tor_fd_setpos(int fd, off_t pos)
{
#ifdef _WIN32
  return real_sgx_lseek(fd, pos, SEEK_SET) < 0 ? -1 : 0;
#else
  return lseek(fd, pos, SEEK_SET) < 0 ? -1 : 0;
#endif
}
...
int
real_tor_ftruncate(int fd)
{
  /* Rumor has it that some versions of ftruncate do not move the file pointer.
   */
  if (real_tor_fd_setpos(fd, 0) < 0)
    return -1;

#ifdef _WIN32
  return real_sgx_chsize(fd, 0);
#else
  return real_sgx_ftruncate(fd, 0);
#endif
}
...
{
  return sgx_connect(socket,address,address_len);
}
                                ...
                                int cloexec, int nonblock)
{
  ...
  s = sgx_socket(domain, type, protocol);
  if (! SOCKET_OK(s))
    return s;
...
//    if (fcntl(s, F_SETFD, FD_CLOEXEC) == -1) {
    if (real_sgx_fcntl(s, F_SETFD, FD_CLOEXEC) == -1) {
      log_warn(LD_FS,"Couldn't set FD_CLOEXEC: %s", strerror(errno));
                                 ...
                                 socklen_t *len, int cloexec, int nonblock)
{
  ...
  s = sgx_accept(sockfd, addr, len);
	tmp = (struct sockaddr_in *)addr;
	printf("Accept = %u.%u.%u.%u:%d\n", addr->sa_data[2]&0xFF,addr->sa_data[3]&0xFF,addr->sa_data[4]&0xFF,addr->sa_data[5]&0xFF, ntohs(tmp->sin_port));
  if (!SOCKET_OK(s))
    return s;
...
set_socket_nonblocking(tor_socket_t socket)
{
  ...
  sgx_ioctlsocket(socket, FIONBIO, (unsigned long*) &nonblocking);
#else
  int flags;

//  flags = fcntl(socket, F_GETFL, 0);
  flags = real_sgx_fcntl(socket, F_GETFL, 0);
  if (flags == -1) {
...
//  if (fcntl(socket, F_SETFL, flags) == -1) {
  if (real_sgx_fcntl(socket, F_SETFL, flags) == -1) {
...
//    r = fcntl(fd[0], F_SETFD, FD_CLOEXEC);
    r = real_sgx_fcntl(fd[0], F_SETFD, FD_CLOEXEC);
    if (r == -1) {
      close(fd[0]);
      close(fd[1]);
      return -errno;
...
//    r = fcntl(fd[1], F_SETFD, FD_CLOEXEC);
    r = real_sgx_fcntl(fd[1], F_SETFD, FD_CLOEXEC);
    if (r == -1) {
      close(fd[0]);
      close(fd[1]);
      return -errno;
...
//  while ((ngids = getgroups(sup_gids_size, sup_gids)) < 0 &&
  while ((ngids = sgx_getgroups(sup_gids_size, sup_gids, 64)) < 0 &&
         errno == EINVAL &&
...
// TODO: smkim, need to check for the third parameter
  if (sgx_setgroups(1, &pw->pw_gid, 1)) {
...
//        (setgid(old_gid) != -1 || setegid(old_gid) != -1)) {
        (sgx_setgid(old_gid) != -1 || sgx_setegid(old_gid) != -1)) {
...
//        (setuid(old_uid) != -1 || seteuid(old_uid) != -1)) {
        (sgx_setuid(old_uid) != -1 || sgx_seteuid(old_uid) != -1)) {
...
make_path_absolute(char *fname)
{
  ...
  char *absfname_malloced = sgx_fullpath(NULL, fname, 1);
  ...
  if (!uname_result_is_set) {
    ...
    if (uname(&u) != -1) {
...
#endif
      {
        ...
        if (! sgx_GetVersionEx((LPOSVERSIONINFO)&info)) {
...
tor_socket_errno(tor_socket_t sock)
{
  ...
  if (err == WSAEWOULDBLOCK && SOCKET_OK(sock)) {
    if (sgx_getsockopt(sock, SOL_SOCKET, SO_ERROR, (void*)&optval, &optvallen))
      return err;
    if (optval)
      return optval;
  }
...
{
#ifdef _WIN32
#ifndef TOR_SGX // SGX: Application already did WSAStartup
  /* This silly exercise is necessary before windows will allow
   * gethostbyname to work. */
  int r;
  r = sgx_WSAStartup(0x101);
  if (r) {
...
{
	char *str = tor_strdup("SGX (format_win32_error): ");
	return str;
#ifndef TOR_SGX // We don't need this function
  TCHAR *str = NULL;
  char *result;
  DWORD n;

  /* Somebody once decided that this interface was better than strerror(). */
  n = sgx_FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER |
  ...
  if (str) {
    sgx_LocalFree((int)str); /* LocalFree != free() */
  }
  return result;
#endif // TOR_SGX
}
...
//  if (-1 == (fd = tor_open_cloexec("/proc/meminfo",O_RDONLY,0)))
  if (-1 == (fd = real_tor_open_cloexec("/proc/meminfo",O_RDONLY,0)))
    return 0;
  s = real_read_file_to_str_until_eof(fd, 65536, &file_size);
...
#ifdef TOR_SGX
{
  ...
  memcpy(tmp, cp+16, 12);
  result = (unsigned long long)strtoll(tmp, &end, 10);
}
#else
 ...
 err:
  ...
  if (! sgx_GlobalMemoryStatusEx(&ms))
    return 0;
...
tor_getpass(const char *prompt, char *output, size_t buflen)
{
  ...
  buf = tor_calloc(buflen, sizeof(wchar_t));
