...
//    sigemptyset(&action.sa_mask);
    sgx_sigemptyset(&action.sa_mask);
    action.sa_handler = SIG_IGN;

    sgx_sigaction(SIGINT,  &action, NULL);
    sgx_sigaction(SIGTERM, &action, NULL);
    sgx_sigaction(SIGPIPE, &action, NULL);
    sgx_sigaction(SIGUSR1, &action, NULL);
    sgx_sigaction(SIGUSR2, &action, NULL);
    sgx_sigaction(SIGHUP,  &action, NULL);
#ifdef SIGXFSZ
    sgx_sigaction(SIGXFSZ, &action, NULL);
/*
...
char *
start_do_list_fingerprint(char *data_dir)
{
	char *retv = (char *)calloc(1, 1024);
	char buf[FINGERPRINT_LEN+1];
  crypto_pk_t *k;
  char *nickname;
	int argc = 8;
	char *argv[8] = {
		"C:\\Users\\INA-SGX-USER\\Desktop\\TorVS2012\\x64\\Debug\\TorVS2012.exe",
		"--list-fingerprint", 
		"--orport", 
		"1",
		"--dirserver",
		"x 127.0.0.1:1 ffffffffffffffffffffffffffffffffffffffff",
		"--datadirectory"
		//"C:/Users/INA-SGX-USER/Desktop/TorVS2012/nodes/A001/"
	};
	argv[7] = data_dir;
  ...
  if (crypto_pk_get_fingerprint(k, buf, 0)<0) {
    log_err(LD_BUG, "Error computing fingerprint");
    return NULL;
  }
	tor_snprintf(retv, strlen(nickname)+strlen(buf)+3, "%s %s\n", nickname, buf);
	printf("%s\n", retv);
...
do_dump_config(void)
{
  ...
  if (!strcmp(arg, "short")) {
    ...
    printf("STDERR: No valid argument to --dump-config found!\n");
    printf("STDERR: Please select 'short', 'non-builtin', or 'full'.\n");
...
#ifdef _WIN32
#ifndef TOR_SGX // This will be execute in app main (DEP)
