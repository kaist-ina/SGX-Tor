...
trusted_dirs_reload_certs(void)
{
  ...
  contents = real_read_file_to_str(filename, RFTS_IGNORE_MISSING, NULL);
  tor_free(filename);
  if (!contents)
    return 0;
...
trusted_dirs_flush_certs_to_disk(void)
{
  ...
  filename = get_datadir_fname("cached-certs");
  if (real_write_chunks_to_file(filename, chunks, 0, 0)) {
                              ...
                              desc_store_t *store)
{
  ...
  if (real_append_bytes_to_file(fname, body, len, 1)) {
...
router_rebuild_store(int flags, desc_store_t *store)
{
  ...
  if (real_write_chunks_to_file(fname_tmp, chunk_list, 1, 1)<0) {
  ...
  if (store->mmap) {
//    int res = real_tor_munmap_file(store->mmap);
  ...
  if (real_replace_file(fname_tmp, fname)<0) {
    log_warn(LD_FS, "Error replacing old router store: %s", strerror(errno));
    goto done;
  }

  errno = 0;
//  store->mmap = real_tor_mmap_file(fname);
  ...
  real_write_str_to_file(fname, "", 1);
...
//  store->mmap = real_tor_mmap_file(fname);
  ...
  if (real_file_status(fname) == FN_FILE)
    contents = real_read_file_to_str(fname, RFTS_BIN|RFTS_IGNORE_MISSING, &st);
  if (contents) {
    if (extrainfo)
