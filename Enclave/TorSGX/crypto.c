...
crypto_pk_base64_encode(const crypto_pk_t *pk, char **priv_out)
{
  ...
  *priv_out = NULL;
  ...
  priv_len = base64_encode_size(der_len, 0) + 1;
...
#ifdef _WIN32
  if (!provider_set) {
    if (!sgx_CryptAcquireContext(&provider, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) {
      log_warn(LD_CRYPTO, "Can't get CryptoAPI provider [1]");
      return -1;
    }
    provider_set = 1;
  }
  if (!sgx_CryptGenRandom(provider, out_len, out)) {
...
//    fd = open(sandbox_intern_string(filenames[i]), O_RDONLY, 0);
    fd = real_sgx_open(sandbox_intern_string(filenames[i]), O_RDONLY, 0);
    if (fd<0) continue;
    log_info(LD_CRYPTO, "Reading entropy from \"%s\"", filenames[i]);
    n = real_read_all(fd, (char*)out, out_len, 0);
    close(fd);
    if (n != out_len) {
      log_warn(LD_CRYPTO,
