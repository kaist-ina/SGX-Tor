...
{
	return sgx_get_windows_conf_root();
...
load_torrc_from_disk(config_line_t *cmd_arg, int defaults_file)
{
	...
	char *torrc = sgx_get_torrc();
	if (defaults_file || torrc == NULL)
		return tor_strdup("");
	return tor_strdup(torrc);
#ifndef TOR_SGX
  char *fname=NULL;
  char *cf = NULL;
...
//     fn = tor_strdup(LOCALSTATEDIR PATH_SEPARATOR "tor");
     fn = tor_strdup(PATH_SEPARATOR "tor");
   }
...
#define VERY_OLD_FILE_AGE (28*24*60*60)
  ...
  if (sgx_stat(sandbox_intern_string(fname), &st)==0 &&
