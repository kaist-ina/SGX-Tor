...
log_tor_version(logfile_t *lf, int reset)
{
  ...
  is_new = lf->fd >= 0 && real_tor_fd_getpos(lf->fd) == 0;
  ...
  if (real_write_all(lf->fd, buf, strlen(buf), 0) < 0) /* error */
    return -1; /* failed */
  return 0;
}
...
#warn "MAXLINE is a very low number; it might not be from syslog.h after all"
    ...
    if (real_write_all(lf->fd, buf, msg_len, 0) < 0) { /* error */
...
tor_log_err_sigsafe_write(const char *s)
{
  ...
  for (i=0; i < n_sigsafe_log_fds; ++i) {
    r = real_sgx_write(sigsafe_log_fds[i], s, len);
    err += (r != (ssize_t)len);
  }
  return err ? -1 : 0;
}
             ...
             const int truncate)
{
  ...
  fd = real_tor_open_cloexec(filename, open_flags, 0644);
  if (fd<0)
    return -1;
