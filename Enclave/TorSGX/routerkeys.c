           ...
           int twice, const or_options_t *options)
{
  ...
  while (1) {
    ...
    if (length != length2 || tor_memneq(buf, buf2, length)) {
      printf("STDERR, That didn't match.\n");
    } else {
      break;
    }
  }
                 ...
                 const char *encrypted_fname)
{
  if (encrypted) {
    ...
    printf("STDERR: Not encrypting the secret key.\n");
  }
...
#define EXPIRES_SOON(cert, interval)            \
  ...
  {
    char *fname = tor_strdup("ed25519_signing");
...

  {
    ...
    if (options->master_key_fname) {
      ...
      fname = tor_strdup("ed25519_master_id");
    }
      ...
      if (need_new_signing_key) {
        ...
        memcpy(&id->pubkey, &check_signing_cert->signing_key,
               sizeof(ed25519_public_key_t));
				fname = tor_strdup("ed25519_master_id_public_key");
