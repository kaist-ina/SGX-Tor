...
//  if (pthread_create(&thread, &attr_detached, tor_pthread_helper_fn, d)) {
  if (sgx_pthread_create(&thread, &attr_detached, tor_pthread_helper_fn, d)) {
    tor_free(d);
    return -1;
  }
...
*/
  int rv;
  rv = (int)sgx_pthread_create(func, 0, data, data_len);
  if (rv == -1)
	return -1;
  return 0;
...
{
  return sgx_pthread_getspecific(threadlocal->key);
...
//  int err = pthread_setspecific(threadlocal->key, value);
  int err = sgx_pthread_setspecific(threadlocal->key, value);
  tor_assert(err == 0);
/*
