...
//  if (real_tor_fd_seekend(fd) < 0)
  ...
  /* Add something about when we opened this file. */
	...
	if (real_write_all(fd, buf, strlen(buf), 0) < 0)
		goto err;
...
keypin_close_journal(void)
{
  if (keypin_journal_fd >= 0)
    real_sgx_close(keypin_journal_fd);
  keypin_journal_fd = -1;
  return 0;
}
                            ...
                            const uint8_t *ed25519_id_key)
{
  if (keypin_journal_fd == -1)
	...
	if (real_write_all(keypin_journal_fd, line, JOURNAL_LINE_LEN, 0)<0) {
...
{
//  tor_mmap_t *map = real_tor_mmap_file(fname);
