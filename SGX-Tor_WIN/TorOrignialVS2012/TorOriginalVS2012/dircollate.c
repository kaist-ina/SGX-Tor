...
{
  return (ddmap_entry_t *)tor_malloc_zero(STRUCT_OFFSET(ddmap_entry_t, vrs_lst) +
...
dircollator_collate_by_ed25519(dircollator_t *dc)
{
  ...
  HT_FOREACH(iter, double_digest_map, &dc->by_both_ids) {
    ...
    vrs_lst2 = (vote_routerstatus_t **)digestmap_get(dc->by_rsa_sha1,
                                                   (char*)ent->d);
    tor_assert(vrs_lst2);
