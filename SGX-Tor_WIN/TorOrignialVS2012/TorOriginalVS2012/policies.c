...
validate_addr_policies(const or_options_t *options, char **msg)
{
  ...
  smartlist_t *addr_policy=NULL;
	 ...
	 int exitrelay_setting_is_auto;
	 int policy_accepts_something;
	 *msg = NULL;
  if (policies_parse_exit_policy_from_options(options,0,NULL,0,&addr_policy)) {
	...
	exitrelay_setting_is_auto = options->ExitRelay == -1;
...
policies_parse_exit_policy_internal(config_line_t *cfg, smartlist_t **dest,
                                    ...
                                    int add_default_policy)
{
	 int found_final_effective_entry ;
	 int first_redundant_entry ;
	 int i ;
  if (!ipv6_exit) {
  ...
  found_final_effective_entry = 0;
  first_redundant_entry = 0;
  for (i = 0; i < smartlist_len(*dest); ++i) {
    sa_family_t family;
    addr_policy_t *p;
    int found_ipv4_wildcard = 0, found_ipv6_wildcard = 0;

    p = (addr_policy_t *)smartlist_get(*dest, i);
