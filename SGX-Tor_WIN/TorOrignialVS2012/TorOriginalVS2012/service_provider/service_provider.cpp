        ...
        (g_sp_extended_epid_group_id != NULL && g_sp_extended_epid_group_id->extended_epid_group_id != extended_epid_group_id))
    {
        ...
        for (size_t i = 0; i < sizeof(g_extended_epid_groups) / sizeof(sample_extended_epid_group); i++)
        {
            if (g_extended_epid_groups[i].extended_epid_group_id == extended_epid_group_id)
            {
                ...
                // registers it exchanges TLS certs with the IAS and obtains an SPID and
                // Report Key from the IAS.
                // For the product IAS, enrollment is an offline process.  See the 'on-boarding'
						...
						ra_samp_response_header_t **pp_msg2)
{
    ...
    do
    {
        ...
        // In the product, the SP and IAS uses an established channel for
        // communication.
        uint8_t* sig_rl;
        uint32_t sig_rl_size = 0;

        // The product interface uses a REST based message to get the SigRL.
        // Refer to the IAS API for more information on how to communicate to
        // the real IAS.  
                        ...
                        ra_samp_response_header_t **pp_att_result_msg)
{
    ...
    do
    {
        ...
        // In the product, the IAS uses a REST message and JSON formatting to request
        // attestation Quote verification.  The sample only simulates this interface.
        // Please refer to the IAS API for more details on this interface.
        FILE* OUTPUT = stdout;
        fprintf(OUTPUT, "\n\n\tAtestation Report:");
        ...
        // Note: This sample always assumes the PIB is sent by the IAS.  In the product
        // implementation, the IAS only sends the PIB for certain attestation 
        ...
        // IAS only verifies the quote structure and signature.  It does not 
