                                ...
                                consensus_flavor_t flavor)
{
  ...
  /* Add the actual router entries. */
  {
    ...
    flag_counts = (int *)tor_calloc(smartlist_len(flags), sizeof(int));
    num_routers = dircollator_n_routers(collator);
    for (i = 0; i < num_routers; ++i) {
      ...
      for (voter_idx = 0; voter_idx < smartlist_len(votes); ++voter_idx) {
        ...
        for (flag = 0; flag < n_voter_flags[voter_idx]; ++flag) {
...
compute_consensus_package_lines(smartlist_t *votes)
{
  ...
  strmap_t *package_status = strmap_new();
	...
	char **status ;
	smartlist_t *entries ;
	smartlist_t *result_list;
	int n_voting_for_entry;
  ...
  SMARTLIST_FOREACH_BEGIN(votes, networkstatus_t *, v) {
    ...
    SMARTLIST_FOREACH_BEGIN(v->package_lines, const char *, line) {
      ...
      key = tor_strndup(line, cp - line);

      status = (char **)strmap_get(package_status, key);
      if (!status) {
        status = (char **)tor_calloc(n_votes, sizeof(const char *));
  ...
  entries = smartlist_new(); /* temporary */
  result_list = smartlist_new(); /* output */
  STRMAP_FOREACH(package_status, key, const char **, values) {
    ...
    n_voting_for_entry = smartlist_len(entries);
    most_frequent =
  ...
  result = smartlist_join_strings(result_list, "", 0, NULL);
