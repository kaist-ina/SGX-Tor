...
rotate_onion_key(void)
{
  ...
  time_t now;
  fname = tor_strdup("secret_onion_key");
	fname_prev = tor_strdup("secret_onion_key.old");
  ...
  tor_free(fname);
  tor_free(fname_prev);
	fname = tor_strdup("secret_onion_key_ntor");
	fname_prev = tor_strdup("secret_onion_key_ntor.old");
                      ...
                      authority_cert_t **cert_out)
{
  ...
  fname = tor_strdup("authority_signing_key");
  ...
  tor_free(fname);
  fname = tor_strdup("authority_certificate");
...
init_keys(void)
{
  ...
  keydir = tor_strdup("secret_id_key");
  ...
  /* 2. Read onion key.  Make it if none is found. */
	keydir = tor_strdup("secret_onion_key");
  ...
  if (options->command == CMD_RUN_TOR) {
	...
	keydir = tor_strdup("secret_onion_key.old");
...

  {
    ...
    int r;
		keydir = tor_strdup("secret_onion_key_ntor");
    ...
    if (r<0)
      return -1;
		...
		keydir = tor_strdup("secret_onion_key_ntor.old");
    if (tor_mem_is_zero((const char *)
                           last_curve25519_onion_key.pubkey.public_key,
                        CURVE25519_PUBKEY_LEN) &&
