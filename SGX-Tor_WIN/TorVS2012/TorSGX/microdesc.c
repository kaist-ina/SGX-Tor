...
dump_microdescriptor(int fd, microdesc_t *md, size_t *annotation_len_out)
{
  ...
  if (md->last_listed) {
    ...
    if (real_write_all(fd, annotation, strlen(annotation), 0) < 0) {
      log_warn(LD_DIR,
  ...
  md->off = real_tor_fd_getpos(fd);
  written = real_write_all(fd, md->body, md->bodylen, 0);
...
{
  smartlist_t *added;
  real_open_file_t *open_file = NULL;
  int fd = -1;
  //  int n_added = 0;
  ssize_t size = 0;
  if (where == SAVED_NOWHERE && !no_save) {
    fd = real_start_writing_to_file(cache->journal_fname,
  ...
  SMARTLIST_FOREACH_BEGIN(descriptors, microdesc_t *, md) {
    ...
    if (fd >= 0) {
      ...
      if (size < 0) {
        ...
        real_abort_writing_to_file(open_file);
        fd = -1;
      } else {
  ...
  if (fd >= 0) {
    if (real_finish_writing_to_file(open_file) < 0) {
...
microdesc_cache_clear(microdesc_cache_t *cache)
{
  ...
  if (cache->cache_content) {
    int res = real_tor_munmap_file(cache->cache_content);
    if (res != 0) {
      log_warn(LD_FS,
...
microdesc_cache_reload(microdesc_cache_t *cache)
{
  ...
  mm = cache->cache_content = real_tor_mmap_file(cache->cache_fname);
  if (mm) {
  ...
  journal_content = real_read_file_to_str(cache->journal_fname,
...
microdesc_cache_rebuild(microdesc_cache_t *cache, int force)
{
  ...
  fd = real_start_writing_to_file(cache->cache_fname,
  ...
  if (cache->cache_content) {
    res = real_tor_munmap_file(cache->cache_content);
    if (res != 0) {
      log_warn(LD_FS,
  ...
  cache->cache_content = real_tor_mmap_file(cache->cache_fname);
  ...
  real_write_str_to_file(cache->journal_fname, "", 1);
  cache->journal_len = 0;
  cache->bytes_dropped = 0;
